    ...
    <div className="p-8">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 rounded-2xl border bg-white p-6">
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-700">{course.name}</span>
            {category && (
          ...
          <p className="mt-4 text-gray-600">コース: {course.description}</p>

          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <InfoRow icon={<Calendar className="h-5 w-5 text-gray-400" />} label="実施日時" value={training.date} />
            <InfoRow icon={<Clock className="h-5 w-5 text-gray-400" />} label="所要時間" value="90分想定" />
            <InfoRow icon={<MapPin className="h-5 w-5 text-gray-400" />} label="場所" value={training.location} />
            <InfoRow
            ...
            <div className="mt-6 rounded-xl border border-dashed p-4">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {training.videos.map((video) => (
                  <div key={video.id} className="rounded-lg border bg-gray-50">
        ...
        <div className="rounded-2xl border bg-white p-6">
          <h3 className="text-lg font-bold text-gray-900">出席状況</h3>
          <div className="mt-4">
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-500">出席率</span>
              <span className="text-3xl font-bold text-gray-900">{checkInRate}%</span>
            </div>
            <div className="mt-2 h-2 rounded-full bg-gray-200">
              <div className="h-2 rounded-full bg-emerald-500" style={{ width: `${checkInRate}%` }} />
            </div>
            <div className="mt-4 grid grid-cols-2 gap-4">
              ...
              <div className="rounded-lg bg-gray-50 p-3 text-center">
                ...
                <p className="text-xs text-gray-500">未出席</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border bg-white">
        <div className="border-b px-6 py-4">
          <h3 className="text-lg font-semibold text-gray-900">参加者リスト</h3>
          <p className="text-sm text-gray-500">チェックイン操作で出席状況を更新できます</p>
        </div>
        <table className="w-full">
          <thead className="border-b bg-gray-50">
            <tr>
              <th className="p-4 text-left text-xs font-medium text-gray-700">名前</th>
              <th className="p-4 text-left text-xs font-medium text-gray-700">メール</th>
              <th className="p-4 text-left text-xs font-medium text-gray-700">会員タイプ</th>
              <th className="p-4 text-left text-xs font-medium text-gray-700">出席状況</th>
              <th className="p-4 text-left text-xs font-medium text-gray-700">チェックイン時刻</th>
              <th className="p-4 text-left text-xs font-medium text-gray-700">操作</th>
            </tr>
          </thead>
          <tbody>
            {participants.map((participant) => (
              <tr key={participant.id} className="border-b last:border-b-0 hover:bg-gray-50">
                <td className="p-4 text-sm font-semibold text-gray-900">{participant.name}</td>
                <td className="p-4 text-sm text-gray-600">{participant.email}</td>
                <td className="p-4">
                  <span
                    className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium ${
                      participant.memberType === "premium" ? "bg-indigo-100 text-indigo-700" : "bg-gray-100 text-gray-700"
                    }`}
                  >
                ...
                <td className="p-4 text-sm text-gray-600">{participant.checkInTime ?? "-"}</td>
                <td className="p-4">
                  {participant.checkedIn ? (
                    <button
                      onClick={() => handleCheckOut(participant.id)}
                      className="flex items-center gap-2 rounded-lg border border-red-200 px-3 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50"
                    >
                      <XCircle className="h-4 w-4" />
                      取消
                    </button>
