...
export function CounterpartiesClient({
  ...
  const openEdit = (item: Counterparty) => {
    ...
    setFormState({
      id: item.id,
      counterpartyId: String(item.counterparty_id),
      name: item.name ?? "",
  ...
  const handleDelete = async (item: Counterparty) => {
    if (!window.confirm(`相手先「${item.name ?? "(名称未設定)"}」を削除しますか？`)) return;

    try {
      const response = await fetch(`/api/internal/counterparties?id=${encodeURIComponent(item.id)}`, {
        method: "DELETE",
      });
      if (!response.ok) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        counterparty_id: formState.counterpartyId,
        name: formState.name,
      ...
      <CardContent className="p-0">
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div className="space-y-1">
            <h2 className="text-sm font-medium leading-none text-foreground">相手先一覧</h2>
            <p className="text-xs text-muted-foreground">相手先マスタの一覧・編集・追加ができます。</p>
          </div>
            ...
            {items.map((item: Counterparty) => (
              <TableRow key={item.id} className="border-b border-border text-sm">
                ...
                <TableCell className="font-mono text-xs text-muted-foreground">
                  <HighlightText text={String(item.counterparty_id)} searchTerm={pagination.search} />
                </TableCell>
