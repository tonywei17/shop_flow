...
export async function POST(req: NextRequest) {
  const spreadsheetId = process.env.GOOGLE_SHEETS_COUNTERPARTIES_SPREADSHEET_ID;
  const sheetName = process.env.GOOGLE_SHEETS_COUNTERPARTIES_SHEET_NAME || "Counterparties";

  if (!spreadsheetId) {
    return NextResponse.json(
      { error: "GOOGLE_SHEETS_COUNTERPARTIES_SPREADSHEET_ID is not configured" },
      { status: 500 },
    );
  }
