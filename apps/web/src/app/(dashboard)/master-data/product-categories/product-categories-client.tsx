...
export function ProductCategoriesClient({
  ...
  const handleBulkDelete = async () => {
    ...
    if (typeof window !== "undefined") {
      const confirmed = window.confirm(`選択中の商品区分（${selectedIds.length}件）を削除しますか？`);
      if (!confirmed) return;
    }
  ...
  const handleExportCsv = (mode: "all" | "selected") => {
    ...
    const base = "/api/internal/product-categories/export-csv";
    const url =
  ...
  const handleExportSheets = async (mode: "all" | "selected") => {
    ...
    try {
      ...
      const response = await fetch("/api/internal/product-categories/export", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <Card className="rounded-xl border bg-card shadow-sm">
      <CardContent className="p-0">
        ...
        <div className="flex flex-col gap-3 border-b border-border px-6 py-3 text-sm text-foreground md:flex-row md:items-center md:justify-between">
          <label htmlFor="product-categories-select-all" className="flex items-center gap-3">
            <Checkbox
              id="product-categories-select-all"
              aria-label="全て選択"
            ...
            {items.map((item: ProductCategory) => (
              <TableRow key={item.id} className="border-b border-border text-sm">
                ...
                <TableCell className="font-mono text-xs text-muted-foreground">
                  <HighlightText text={item.code ?? ""} searchTerm={pagination.search} />
                </TableCell>
