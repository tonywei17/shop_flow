...
| ⑤ | 教材销售返还 | 教材販売割戻し | **只计算教室购买的订单**マージン（一般価格×数量×マージン比例） |
...
|------|------|--------|------|
| 普通教室 | 480円 | 无 | 正常计算 |
| アイグラン教室（777） | 480円 | 600円/人 | 有割戻し，店番尾号777（㈱アイグラン） |
| アイグラン教室（778） | 480円 | 600円/人 | 有割戻し，店番尾号778（㈱アイグラン あい福祉会） |
...
materialReturn = materials.reduce((sum, m) => {
  ...
  const margin = (m.price_retail * m.quantity) * marginRate;  // マージン比例从支局配置获取
...
アイグラン教室有割戻し（返还），包括两种类型：

| 店番尾号 | 教室名 | 单价 | 割戻し |
|----------|--------|------|--------|
| 777 | ㈱アイグラン | 480円/人 | 600円/人 |
| 778 | ㈱アイグラン あい福祉会 | 480円/人 | 600円/人 |

割戻し金额在CC会費明细的最后一列显示，并从顶部②チャイルドクラブ会費中扣除。

**判断逻辑**：`classroom_code.endsWith("777") || classroom_code.endsWith("778")`
...
| 2025-12-19 | **マージン計算**：教室购买的マージン = (一般価格 × 数量) × マージン比例 |
| 2025-12-19 | **CC会员数据修复**：支局自己的数据不受is_excluded过滤影响 |
| 2025-12-19 | **纳入先显示修复**：支局自己（以000结尾）显示为"00"而不是"000" |
| 2025-12-19 | **アイグラン教室扩展**：支持778尾号教室（㈱アイグラン あい福祉会）也有割戻し |
| 2025-12-19 | **收件人地址显示**：邮编/都道府县/市区町村/番地/建物/责任者分行显示，支局名不换行 |
| 2025-12-19 | **发送者注册号显示**：注册番号「T6011005001316」禁止换行 |
| 2025-12-19 | **请求明细分页**：按最大表格高度730px分页，行高按内容长度估算（长文本行高翻倍），绿色横条紧贴表格底部 |
| 2025-12-19 | **0人教室过滤**：PDF明细中移除人数为0的教室行，保持HTML一致 |
