# 生産環境デプロイメント実行チェックリスト
> **作成日**: 2025-12-16  
> **対象**: shop_flow v1.0 本番環境更新  
> **実行者**: [記入してください]

---

## 📋 デプロイ前チェック

### ローカル環境確認
- [ ] Git リポジトリが最新状態か確認
  ```bash
  git status
  git log --oneline -5
  ```
  **期待値**: `On branch main`, `Your branch is up to date with 'origin/main'`

- [ ] ワーキングツリーがクリーンか確認
  ```bash
  git status
  ```
  **期待値**: `nothing to commit, working tree clean`

- [ ] ビルドが成功しているか確認
  ```bash
  pnpm build
  ```
  **期待値**: すべてのタスクが成功

- [ ] Lint チェックが成功しているか確認
  ```bash
  pnpm lint
  ```
  **期待値**: エラーなし

### 本番環境事前確認
- [ ] サーバーへの SSH 接続確認
  ```bash
  ssh root@100.112.168.22 "echo 'Connection OK'"
  ```

- [ ] プロジェクトディレクトリが存在するか確認
  ```bash
  ssh root@100.112.168.22 "ls -la /root/shop_flow"
  ```

- [ ] Docker Compose が利用可能か確認
  ```bash
  ssh root@100.112.168.22 "docker compose version"
  ```
  **期待値**: v2.35.1 以上

- [ ] 現在のコンテナ状態を確認
  ```bash
  ssh root@100.112.168.22 "docker ps --filter 'name=shop_flow'"
  ```

- [ ] ディスク容量を確認
  ```bash
  ssh root@100.112.168.22 "df -h /"
  ```
  **期待値**: 20GB 以上の空き容量

### データベース確認
- [ ] PostgreSQL が正常に動作しているか確認
  ```bash
  ssh root@100.112.168.22 "docker logs shop_flow-postgres --tail 20"
  ```

- [ ] 最新のマイグレーションが適用されているか確認
  ```bash
  ssh root@100.112.168.22 "docker exec shop_flow-postgres psql -U shop_flow -d shop_flow -c '\dt'"
  ```

- [ ] データベースバックアップを取得
  ```bash
  ssh root@100.112.168.22 "docker exec shop_flow-postgres pg_dump -U shop_flow shop_flow > /root/shop_flow_backup_$(date +%Y%m%d_%H%M%S).sql"
  ```

### SSL 証明書確認
- [ ] Traefik ログで証明書エラーがないか確認
  ```bash
  ssh root@100.112.168.22 "docker logs traefik --tail 50 | grep -i 'error\|certificate'"
  ```

- [ ] DNS 設定が正しいか確認
  ```bash
  ssh root@100.112.168.22 "dig eurhythmics.yohaku.cloud"
  ```

---

## 🚀 デプロイメント実行

### ステップ 1: サーバーに接続
```bash
ssh root@100.112.168.22
```
- [ ] 接続成功

### ステップ 2: プロジェクトディレクトリに移動
```bash
cd /root/shop_flow
```
- [ ] ディレクトリ移動成功

### ステップ 3: 最新コードを取得
```bash
git fetch origin
git reset --hard origin/main
```
- [ ] Git fetch 成功
- [ ] Git reset 成功
- [ ] ログで最新コミット確認: `dc9ad81 chore: sync latest change`

### ステップ 4: Docker イメージをビルド
```bash
docker compose -f docker-compose.prod.yml build --no-cache web storefront learning
```
- [ ] web イメージビルド成功
- [ ] storefront イメージビルド成功
- [ ] learning イメージビルド成功
- [ ] ビルド時間を記録: _____ 分

### ステップ 5: コンテナを再起動
```bash
docker compose -f docker-compose.prod.yml up -d web storefront learning
```
- [ ] web コンテナ起動成功
- [ ] storefront コンテナ起動成功
- [ ] learning コンテナ起動成功

### ステップ 6: 初期状態確認
```bash
docker ps --filter 'name=shop_flow' --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'
```
- [ ] すべてのコンテナが `Up` 状態
- [ ] ポートマッピングが正しい

---

## 🧪 デプロイ後の検証

### ヘルスチェック
```bash
# 管理後台
curl -s -o /dev/null -w '%{http_code}' https://eurhythmics.yohaku.cloud/
```
- [ ] HTTP ステータス: 200 (期待値)
- [ ] 記録時刻: _____

```bash
# ストアフロント
curl -s -o /dev/null -w '%{http_code}' https://eurhythmics-shop.yohaku.cloud/
```
- [ ] HTTP ステータス: 200 (期待値)
- [ ] 記録時刻: _____

```bash
# Learning プラットフォーム
curl -s -o /dev/null -w '%{http_code}' https://e-learning.yohaku.cloud/
```
- [ ] HTTP ステータス: 200 (期待値)
- [ ] 記録時刻: _____

### ログ確認
```bash
docker logs shop_flow-web --tail 50
```
- [ ] エラーなし
- [ ] 起動ログが正常

```bash
docker logs shop_flow-storefront --tail 50
```
- [ ] エラーなし
- [ ] 起動ログが正常

```bash
docker logs shop_flow-learning --tail 50
```
- [ ] エラーなし
- [ ] 起動ログが正常

### リソース使用量確認
```bash
docker stats --filter 'name=shop_flow' --no-stream
```
- [ ] CPU 使用率: _____ %
- [ ] メモリ使用率: _____ %
- [ ] 異常な使用量がないか確認

### データベース接続確認
```bash
docker exec shop_flow-postgres pg_isready -U shop_flow
```
- [ ] 接続成功 (出力: `accepting connections`)

---

## 🌐 機能テスト

### 管理後台 (Dashboard)
1. [ ] https://eurhythmics.yohaku.cloud にアクセス
2. [ ] ログインページが表示される
3. [ ] 管理者アカウントでログイン成功
4. [ ] ダッシュボードが表示される
5. [ ] サイドバーメニューが表示される
6. [ ] 商品管理ページにアクセス可能
7. [ ] 商店設定ページにアクセス可能
8. [ ] ロール管理ページにアクセス可能

### ストアフロント (Storefront)
1. [ ] https://eurhythmics-shop.yohaku.cloud にアクセス
2. [ ] ホームページが表示される
3. [ ] 商品一覧が表示される
4. [ ] 商品検索が機能する
5. [ ] 商品詳細ページにアクセス可能
6. [ ] カートに商品を追加可能
7. [ ] チェックアウトフローが正常

### Learning プラットフォーム
1. [ ] https://e-learning.yohaku.cloud にアクセス
2. [ ] ホームページが表示される
3. [ ] ログインページが表示される
4. [ ] ソーシャルログインボタンが表示される
5. [ ] アクティビティページにアクセス可能
6. [ ] Vimeo 動画プレイヤーが表示される
7. [ ] AI アシスタントパネルが表示される

---

## 📊 パフォーマンス確認

### ページロード時間
```bash
curl -w "@curl-format.txt" -o /dev/null -s https://eurhythmics.yohaku.cloud/
```
- [ ] Total time: _____ 秒 (目標: < 3秒)

### API レスポンス時間
```bash
curl -w "%{time_total}\n" -o /dev/null -s https://eurhythmics.yohaku.cloud/api/auth/me
```
- [ ] Response time: _____ 秒 (目標: < 1秒)

---

## 🔍 セキュリティ確認

- [ ] HTTPS が有効か確認
  ```bash
  curl -I https://eurhythmics.yohaku.cloud | grep -i "strict-transport-security"
  ```

- [ ] SSL 証明書が有効か確認
  ```bash
  openssl s_client -connect eurhythmics.yohaku.cloud:443 -servername eurhythmics.yohaku.cloud < /dev/null | grep -A 2 "Verify return code"
  ```

- [ ] セキュリティヘッダーが設定されているか確認
  ```bash
  curl -I https://eurhythmics.yohaku.cloud | grep -E "X-Frame-Options|X-Content-Type-Options|Content-Security-Policy"
  ```

---

## 📝 デプロイ結果記録

### デプロイ実行情報
- **実行日時**: _____
- **実行者**: _____
- **デプロイ方法**: ☐ ワンライナー ☐ ステップバイステップ
- **所要時間**: _____ 分

### ビルド情報
- **ビルド開始時刻**: _____
- **ビルド終了時刻**: _____
- **ビルド成功**: ☐ はい ☐ いいえ
- **ビルドエラー**: _____

### デプロイ結果
- **デプロイ成功**: ☐ はい ☐ いいえ
- **問題発生**: ☐ はい ☐ いいえ
- **問題内容**: _____

### 検証結果
- **ヘルスチェック**: ☐ 成功 ☐ 失敗
- **機能テスト**: ☐ 成功 ☐ 失敗
- **パフォーマンス**: ☐ 正常 ☐ 低下
- **セキュリティ**: ☐ OK ☐ 問題あり

### 備考
_____

---

## 🔙 ロールバック手順 (問題発生時)

### ロールバック実行
```bash
ssh root@100.112.168.22 "cd /root/shop_flow && \
  git reset --hard <previous-commit-hash> && \
  docker compose -f docker-compose.prod.yml build --no-cache web storefront learning && \
  docker compose -f docker-compose.prod.yml up -d web storefront learning"
```

- [ ] ロールバック実行
- [ ] コンテナ再起動確認
- [ ] ヘルスチェック実行
- [ ] 機能テスト実行

### ロールバック記録
- **ロールバック実行日時**: _____
- **ロールバック対象コミット**: _____
- **ロールバック理由**: _____
- **ロールバック成功**: ☐ はい ☐ いいえ

---

## 📞 サポート連絡先

問題が発生した場合：

1. **ログを確認**
   ```bash
   docker logs shop_flow-web --tail 100
   docker logs shop_flow-storefront --tail 100
   docker logs shop_flow-learning --tail 100
   ```

2. **サーバーリソースを確認**
   ```bash
   docker stats --filter 'name=shop_flow'
   df -h /
   free -h
   ```

3. **データベース接続を確認**
   ```bash
   docker exec shop_flow-postgres pg_isready -U shop_flow
   ```

4. **Traefik ログを確認**
   ```bash
   docker logs traefik --tail 50
   ```

---

**チェックリスト完了日**: _____  
**確認者署名**: _____
