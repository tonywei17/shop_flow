...
export const GET = createApiHandler(async (ctx) => {
  const { searchParams, pagination } = ctx;
  ...
  const sortOrder =
    searchParams.get("order") === "desc" ? "desc"
...
export const POST = createApiHandler(async (ctx) => {
  ...
  if (partialParsed.success && partialParsed.data.is_active !== undefined) {
    ...
    return successResponse({ product: updated, mode: "edit" });
  }

  const data = await ctx.body(productUpsertSchema);
  const mode = data.mode ?? "create";
  const id = data.id?.trim?.() ?? "";

  if (mode === "edit") {
    if (!id) return errorResponse("Missing id for edit mode", 400);
  ...
  return successResponse({ product: created, mode: "create" });
});

export const DELETE = createApiHandler(async (ctx) => {
  const id = ctx.searchParams.get("id")?.trim();
