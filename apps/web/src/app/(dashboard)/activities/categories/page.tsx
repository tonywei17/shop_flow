...
const initialCategories: TrainingCategory[] = [
  { id: "basic", name: "基礎研修", description: "未経験者向け", colorClass: "bg-indigo-100 text-indigo-700", sortOrder: 1, isActive: true },
  { id: "instructor", name: "指導者向け", description: "中級・上級者向け", colorClass: "bg-purple-100 text-purple-700", sortOrder: 2, isActive: true },
  { id: "cert", name: "資格対策", description: "資格試験対策", colorClass: "bg-amber-100 text-amber-700", sortOrder: 3, isActive: true },
];

export default function TrainingCategoriesManagementPage() {
  const [categories, setCategories] = useState<TrainingCategory[]>(initialCategories);
  ...
  const handleStartEdit = (category: TrainingCategory) => {
                    ...
                    return (
                      <tr key={category.id} className="border-b last:border-b-0 hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-sm">
                          <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium ${category.isActive ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-700"}`}>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-lg border p-6">
            ...
            <ul className="text-sm text-gray-700 space-y-2">
              <li>・分類は研修作成画面で選択できます。</li>
              <li>・無効にした分類は新規研修で選択できない状態を想定。</li>
              <li>・将来的には API と同期し、ここでの設定が本番にも反映されます。</li>
            </ul>
          </div>
        </div>
      </div>
